...
interface validatedUser extends Request {
    user: {}
}
...
export const DecryptValidationToken = (req: Request, res: Response, next: NextFunction) => {
    const bearerHeader = req.headers['authorization'];
    if (!bearerHeader) return res.status(403).json({ message: 'token required', status: 'error' });
    try {
        const bearer = bearerHeader.split(' ');
        ...
        if (typeof (data) === "object" && "id" in data) {
            (req as validatedUser).user = data as validatedUser;
            return next();
        } else {
            res.status(400).json({ status: "error", message: "an error occured while decoding token" });
        }
